...
export const ContestApproveAsset = (props: { contractAddress: string }) => {
    ...
    const [userAllowance, setUserAllowance] = useState<BigNumber|undefined>(undefined);
    ...
    const { writeContract, transaction } = useContractWriteApprove(asset.value!, props.contractAddress, userAllowance!)
        ...
        <div className="w-full flex flex-col justify-start items-start gap-2">
            ...
            <div className="py-4 w-full flex flex-col justify-start items-start gap-4 text-slate-100">
                ...
                <ContestContractWriteView 
                    label="Approve amount [wei]"
                    name="Click here to approve"
                    value={ (userAllowance !== undefined) ? userAllowance.toString() : "" }
                    setValue={ (event) => setUserAllowance(BigNumber.from(event.target.value)) }
